...
import { Select, MenuItem, FormControl, List, ListItem, ListItemText, IconButton, TextField } from "@mui/material";
  ...
  const fetchCharacteristics = async (authToken) => {
    try {
      const response = await axios.get(
        "http://ec2-13-60-83-13.eu-north-1.compute.amazonaws.com/characteristics",
        {
    ...
    try {
      ...
      const responses = await Promise.all(
        questionsForApi.map((question) =>
          axios.post(
            "http://ec2-13-60-83-13.eu-north-1.compute.amazonaws.com/questions",
            question,
            {
              headers: {
      ...
      const testResponse = await axios.post(
        "http://ec2-13-60-83-13.eu-north-1.compute.amazonaws.com/tests",
        {
          title: testTitle,
